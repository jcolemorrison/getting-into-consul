...
apt update && apt install -y consul=1.11.1 unzip
...
service {
  ...
  name = "database"
  port = 5432
  token = "" # put database service token here

  check {
    id = "database"
    name = "HTTP API on Port 5432"
...
# Put database service token here for the -token option!
[Service]
ExecStart=/usr/bin/consul connect envoy -sidecar-for=database -token=database_service_token
ExecStop=/bin/sleep 5
Restart=always
