...
# Client CTS Certificates
resource "tls_private_key" "client_cts_key" {
	algorithm = "ECDSA"
	ecdsa_curve = "P256"
}

## Public Client Cert
resource "tls_cert_request" "client_cts_cert" {
	private_key_pem = tls_private_key.client_cts_key.private_key_pem
...
## Signed Public Client Certificate
resource "tls_locally_signed_cert" "client_cts_signed_cert" {
	cert_request_pem = tls_cert_request.client_cts_cert.cert_request_pem
